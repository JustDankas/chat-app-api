...
router.post('/find',async (req:Request<{},{},{requester:string,searchTerm:string},{}>,res:Response)=>{
    try {
        const users = await User.find({$and:[{username:{$ne:req.body.requester}},{username:{$regex:req.body.searchTerm}}]},{password:0})
        res.status(200).json({users})
...
router.put('/:userId',async (req:Request<{userId:string},{},{newUsername:string},{}>,res:Response)=>{
    try {
        const {userId} = req.params
        const {newUsername} = req.body
        const existsUsername = await User.findOne({username:newUsername})
        if(!existsUsername){
            const user = await User.findByIdAndUpdate(userId,{username:newUsername},{new:true}).select({email:1,username:1,id:1})
            if(user){
...
router.post('/register',validateCredentials,async (req:Request<{},{},IUser,{}>,res:Response)=>{
    try {
...
async function validateCredentials(req:Request<{},{},IUser,{}>,res:Response,next:NextFunction){
